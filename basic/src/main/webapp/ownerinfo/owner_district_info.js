...
function initDataGrid() {
	$(datagridJQueryFind).datagrid({
		loadMsg:"数据加载中，请稍后……",
		url : ownerRequestUrl+'/findAllOwnerDistrictInfo.json',
		toolbar : [{
			text : '添加',
			iconCls : 'icon-add',
			handler : function() {
				formsubmitUrl = ownerRequestUrl+'/saveOwnerDistrictInfo.json'
				setDialog("#dlgDIV", '','新增');
			...
			handler : function() {
				...
				if (row) {
					...
					formsubmitUrl = ownerRequestUrl+'/updateOwnerDistrictInfo.json'
				} else {
					...
					$.messager.confirm('确认','您确认想要删除记录吗？',function(r) {
						if (r) {
							$.ajax({
								type : "POST",
								dataType : 'html',
								url : ownerRequestUrl+'/deleteOwnerDistrictInfo.json?ids='+ids,
