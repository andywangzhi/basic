...
$(function() {
	...
	$("#addDIV").hide();
	$(datagridJQueryFind).datagrid('options').url = '/basicinfo/admin/mainAndAffiliatedInfomationQuery/queryMainAndAffiliatedInfomationByPage.json?type=building&detail_location_flag="true"';
	var param = $.fn.getFormParams(searchFrom);
	$(datagridJQueryFind).datagrid('options').queryParams = param;
	$(datagridJQueryFind).datagrid('reload');
...
function submitForm() {
	$('#saveAuthForm')
			.form(
					'submit',
					{
						url : "/basicinfo/admin/mainAndAffiliatedInfomationManage/saveOrUpdateMainAndAffiliatedInfomation.json?type=building",
						ajax : true,
						onSubmit: function(){
...
function submitForm2() {
	$('#saveAuthForm')
			.form(
					'submit',
					{
						url : "/basicinfo/admin/mainAndAffiliatedInfomationManage/saveOrUpdateMainAndAffiliatedInfomation.json?type=building&batch_add_flag=true",
						ajax : true,
						onSubmit: function(){
							var isValid = $(this).form('validate');//校验是否所有的字段是否合规
							if (isValid){
									...
									handler : function() {
										...
										if (getRows.length == 0) {
											...
											$.messager
													.confirm(
															'确认',
															'删除楼宇，会导致楼宇下的楼层、单元、附属、及区域位置的关联信息删除，您确认想要删除记录吗？',
															function(r) {
																if (r) {
																	$
																			.ajax({
																				type : "POST",
																				dataType : 'html',
																				url : '/basicinfo/admin/mainAndAffiliatedInfomationManage/deleteMainAndAffiliatedInfomation.json?type=building&ids='
																						+ ids,
																				success : function(
																						data) {
																					var obj = eval('('
