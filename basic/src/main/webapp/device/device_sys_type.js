...
$(function() {
	...
	//初始化下拉框
	$("#gTypeName_combox").combobox({
		multiple:false,
		editable:false,
		panelHeight:'100px',
		url: baseDeviceTypeRequestUrl+'/gTypeCombox.json',
		valueField:'id',    
	    textField:'text',
	    method:'GET'
	});
...
function initDataGrid() {
	$(datagridJQueryFind).datagrid({
		loadMsg:"数据加载中，请稍后……",
		url : baseDeviceTypeRequestUrl+'/findAllSysType.json',
		toolbar : [{
			text : '添加',
			iconCls : 'icon-add',
			handler : function() {
				formsubmitUrl = baseDeviceTypeRequestUrl+'/saveSysType'
				setDialog("#dlgDIV", '','新增');
			...
			handler : function() {
				...
				if (row) {
					...
					$("#dlgDIV").show();
					clearFrom(dataFormDiv);
					console.log(JSON.stringify(row.goalType));
					$("#gTypeName_combox").combobox("select",row.goalType.id);
					//加载数据
					$(dataFormDiv).form('load',row);
					formsubmitUrl = baseDeviceTypeRequestUrl+'/updateSysType.json'
				} else {
			...
			handler : function() {
				...
				if (rows.length == 0 ) {
					...
					$.messager.confirm('确认','您确认想要删除该类型及该类型(系统类型->品牌|自定义->设备信息)所有子类吗？',function(r) {
						if (r) {
							$.ajax({
								type : "POST",
								dataType : 'html',
								url : baseDeviceTypeRequestUrl+'/deleteSysType.json?ids='+ids,
