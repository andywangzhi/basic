...
$(function() {
	...
	//初始化下拉框
	$("#sTypeName_combox").combobox({
		multiple:false,
		editable:false,
		panelHeight:'100px',
		url: baseDeviceTypeRequestUrl+'/sTypeCombox.json',
		valueField:'id',    
	    textField:'text',
	    method:'GET'
	});
	$("#q_sTypeName_combox").combobox({
		multiple:false,
		editable:false,
		panelHeight:'100px',
		url: baseDeviceTypeRequestUrl+'/sTypeCombox.json',
...
function initDataGrid() {
	$(datagridJQueryFind).datagrid({
		loadMsg:"数据加载中，请稍后……",
		url : baseDeviceTypeRequestUrl+'/findAllBrand.json',
		toolbar : [{
			text : '添加',
			iconCls : 'icon-add',
			handler : function() {
				formsubmitUrl = baseDeviceTypeRequestUrl+'/saveBrand.json'
				setDialog("#dlgDIV", '','新增');
			...
			handler : function() {
				...
				if (row) {
					...
					$("#dlgDIV").show();
					clearFrom(dataFormDiv);
					$("#sTypeName_combox").combobox("select",row.sysType.id);
					//加载数据
					$(dataFormDiv).form('load',row);
					formsubmitUrl = baseDeviceTypeRequestUrl+'/updateBrand.json'
				} else {
			...
			handler : function() {
				...
				if (rows.length == 0 ) {
					...
					$.messager.confirm('确认','您确认想要删除该类型及该类型(品牌->设备信息)所有子类吗？',function(r) {
						if (r) {
							$.ajax({
								type : "POST",
								dataType : 'html',
								url : baseDeviceTypeRequestUrl+'/deleteBrand.json?ids='+ids,
